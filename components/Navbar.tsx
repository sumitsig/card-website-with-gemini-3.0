    ...
    >
      <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-16 flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2 cursor-pointer group">
          ...
          <span className="text-2xl font-bold tracking-tighter text-gray-900 dark:text-white font-[Space_Grotesk]">
      ...
      {/* Mobile Menu */}
      {mobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 w-full bg-white/95 dark:bg-black/95 border-b border-black/5 dark:border-white/10 backdrop-blur-xl transition-colors">
          <div className="flex flex-col p-6 space-y-4">
            {NAV_ITEMS.map((item) => (
              <a
                key={item.label}
                href={item.href}
                className="text-lg font-medium text-gray-800 dark:text-gray-300 hover:text-orange-600 dark:hover:text-white"
                onClick={() => setMobileMenuOpen(false)}
              >
                {item.label}
              </a>
            ))}
            <button className="w-full py-3 mt-4 text-center font-bold uppercase bg-orange-600 text-white rounded-sm">
              Client Login
            </button>
          </div>
        </div>
